...
export default class Chat extends Component {
  ...
  componentDidMount() {
    ...
    if(!this.propsactiveChannel){
      dispatch(actions.fetchMessages('Lobby'));
    }
  }
    ...
    const dropDownMenu = (
      <div style={{ 'width': '21rem', 'top': '0', alignSelf: 'baseline', padding: '0', margin: '0', order: '1' }}>
        <DropdownButton key={1} style={{ 'width': '21rem' }} id="user-menu" bsSize="large" bsStyle="primary" title={username}>
          <MenuItem style={{ 'width': '21rem' }} eventKey="4" onSelect={this.handleSignOut.bind(this)}>Sign out</MenuItem>
        </DropdownButton>
      </div>
    );
    const PrivateMessageModal = (
      <div>
        <Modal bsSize="small" key={1} show={this.state.privateChannelModal} onHide={this.closePrivateChannelModal.bind(this)}>
          <Modal.Header>
    ...
    const bigNav = (
      <div className="nav">
        ...
        <section style={{ order: '2', marginTop: '1.5em' }}>
          <Channels socket={socket} onClick={this.changeActiveChannel.bind(this)} channels={channels} messages={messages} dispatch={dispatch} user={user.username} />
        </section>
      </div>
    );
    return (
      <div style={{ margin: '0', padding: '0', height: '100%', width: '100%', display: '-webkit-box' }}>
        {bigNav}
        <div className="main">
          <header style={{ background: '#529ada', color: 'white', flexGrow: '0', order: '0', fontSize: '2.3em', paddingLeft: '0.2em' }}>
            <div>
              {activeChannel}
            </div>
          </header>
          {PrivateMessageModal}
          <div style={{ wordWrap: 'break-word', margin: '0', overflowY: 'auto', padding: '0', paddingBottom: '1em', flexGrow: '1', order: '1' }} ref="messageList">
            {filteredMessages.map(message =>
              <MessageListItem handleClickOnUser={this.handleClickOnUser.bind(this)} message={message} key={message.id} user={user} />
            )}
          </div>
